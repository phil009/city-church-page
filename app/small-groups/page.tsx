...
const accordionStrips = [
    { src: congregationBelievers,    label: "Community" },
    { src: outdoorsChoirGroup,       label: "Worship"   },
    { src: congregationPeaceDaniels, label: "Praise"    },
    { src: outdoorsFirstImpression,  label: "Growth"    },
];
...
export default function SmallGroupsPage() {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <GlobalHero
                backgroundImage={
                    groupLifeBg || "/placeholder.svg?height=600&width=1600"
                }
                title="Small Groups"
                breadcrumbs={[
                    { label: "Home", href: "/" },
            ...
            <section className="py-16 relative bg-white overflow-x-hidden">
                <div className="px-4 sm:px-12 md:px-20 md:flex text-center md:text-start md:items-center gap-10">
                    <motion.div
                    ...
                    >
                        {/* Mobile: 2×2 grid */}
                        <div className="grid grid-cols-2 gap-2 h-[300px] md:hidden">
                            {accordionStrips.map((strip, i) => (
                                <div key={i} className="relative rounded-xl overflow-hidden">
                                    <Image src={strip.src} alt={strip.label} fill className="object-cover" />
                                    <div className="absolute inset-0 bg-appDark/30" />
                                </div>
                            ))}
                        </div>
                            ...
                            {accordionStrips.map((strip, i) => (
                                <div
                                    ...
                                    className={`relative overflow-hidden rounded-xl cursor-pointer transition-all duration-500 ease-in-out ${
                                        activeStrip === i ? "flex-[3]" : "flex-[1]"
                                    }`}
                                >
                                    <Image src={strip.src} alt={strip.label} fill className="object-cover" />
                                    <div className={`absolute inset-0 transition-opacity duration-300 ${activeStrip === i ? "bg-appDark/20" : "bg-appDark/50"}`} />

                                    {/* Expanded label */}
                                    <p className={`absolute bottom-5 left-4 text-white font-semibold text-sm tracking-wide transition-opacity duration-300 ${activeStrip === i ? "opacity-100" : "opacity-0"}`}>
                                        {strip.label}
                                    </p>

                                    {/* Collapsed rotated label */}
                                    <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${activeStrip === i ? "opacity-0" : "opacity-70"}`}>
                                        <p className="text-white text-xs font-medium -rotate-90 whitespace-nowrap tracking-widest uppercase">
                                            {strip.label}
                                        </p>
                                    </div>
                                </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        ...
                        >
                            ...
                            <p className="text-gray-100 opacity-70">
                                Form meaningful relationships beyond Sunday
                                services. Small groups provide the perfect
                                setting to build lasting friendships.
                            </p>
                        </motion.div>
                        ...
                        >
                            ...
                            <p className="text-gray-100 opacity-70">
                                Dive deeper into God&apos;s Word in an
                                interactive setting where questions are welcomed
                        ...
                        >
                            ...
                            <p className="text-gray-100 opacity-70">
                                Experience the power of community as you share
                                life&apos;s joys and challenges with people who
            ...
            >
                ...
                />
                <div className="container mx-auto px-4">
                    <h2 className="text-xl md:text-3xl font-bold text-center text-appOffWhite relative mb-12">
                        Available Groups
                    </h2>
